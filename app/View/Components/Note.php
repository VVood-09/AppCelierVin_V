        ...
        <div>
          <template x-for="(star, index) in ratings" :key="index">
            <button @click="rate(star.amount)" @mouseover="hoverRating = star.amount" @mouseleave="hoverRating = rating"
              aria-hidden="true"
              :title="star.label"
              class="rounded-sm text-gray-400 fill-current focus:outline-none focus:shadow-outline p-1 w-12 m-0 cursor-pointer etoile_dimension"
              :class="{'text-gray-600': hoverRating >= star.amount, 'etoile_jaune': rating >= star.amount && hoverRating >= star.amount}">
