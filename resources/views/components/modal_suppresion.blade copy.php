<div class="modal_btn-delete" x-data="{ modalouvert: false }">
  <button @click="modalouvert = true" class="btn-reverse">{{ $triggerText }}</button>
  <div class="" x-show="modalouvert" x-init="$watch('modalouvert', value => { if (value) { document.body.classList.add('no-scroll'); } else { document.body.classList.remove('no-scroll'); } })">
    <div class="modal">
