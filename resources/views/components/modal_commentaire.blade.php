...
<div class="modal_btn-add" x-data="{ ismodalopen: false }">
  ...
  <div class="" x-show="ismodalopen"  @keydown.escape.window="ismodalopen = false"  x-init="$watch('ismodalopen', value => { if (value) { document.body.classList.add('pas-defilement'); } else { document.body.classList.remove('pas-defilement'); } })">
    ...
    <div class="modal ">
      <div class="modal_text">Commentez ce vin</div>
      
      <div class="modal_confirm-btn">
        <form class="formBtl_form " method="post" action="{{ $route }}" onsubmit="return false"> 
               
          <textarea aria-label="commentaire" class="modal_commentaire-textarea" id="commentaire"  placeholder="Écrivez votre commentaire..."></textarea>

          <div class="modal_btn">
            
            <button aria-label="commentaire" @click="ismodalopen = false" class="btn-reverse btn-close">
            ...
            <button aria-label="submit" type="submit"  class="btn" @click="ajoutComment()" x-on:click="ismodalopen = false">
